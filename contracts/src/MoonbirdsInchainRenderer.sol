...
import {ITokenURIGenerator, IMoonbirds} from "contracts/lib/moonbirds-contract/contracts/Moonbirds.sol";

import {IFeaturesProvider} from "./types/IFeaturesProvider.sol";
import {Attribute} from "./types/Attribute.sol";
import {Mutators, IMutatorsProvider} from "./types/IMutatorsProvider.sol";

import {BMP} from "../ethier/utils/BMP.sol";
import {Image} from "../ethier/utils/Image.sol";

import {Assembler} from "./Assembler.sol";
import {BackgroundRegistry} from "./BackgroundRegistry.sol";

import {Features, FeaturesLib} from "./gen/Features.sol";
    ...
    IFeaturesProvider
{
    ...
    constructor(
        IMoonbirds moonbirds_, // 0x23581767a106ae21c074b2276D25e5C3e136a68b
        IFeaturesProvider userRegistry_, // 0xF8D83845DEb59EE43CF012e57731209A472baF8c
        BackgroundRegistry backgroundRegistry_, // 0xB7f6618F9E20fb56d5337000a90e540F4005c696
        Assembler assembler_, // 0xCc80f29a7DB70d727a666AE46Cf4CC3179514368
        string memory offchainBaseURI_ // "https://proof.xyz/moonbirds/"
    ) {
    ...
    function getFeatures(
        uint256 tokenId
    ) public view returns (Features memory) {
    ...
    function getMutators(
        uint256 tokenId
    ) public view returns (Mutators memory) {
...
//        Attribute[] memory attrs = assembler.assembleAttributes(features);
    ...
    function setProofRegistry(
        IFeaturesProvider proofRegistry_
    ) external onlyOwner {
     ...
     * renouncing contract ownership.
     */
    function setBackgroundRegistry(
        BackgroundRegistry backgroundRegistry_
    ) external onlyOwner {
        backgroundRegistry = backgroundRegistry_;
    }
     ...
     * renouncing contract ownership.
     */
    function setExternalLinkBaseURL(
        string memory externalLinkBaseURL_
    ) external onlyOwner {
