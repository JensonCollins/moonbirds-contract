...
import {BucketCoordinates} from "../../solidify/BucketStorageLib.sol";
...
library LayerStorageMapping {
    ...
    function locate(
        LayerType layerType,
        uint256 index
    ) internal pure returns (StorageCoordinates memory) {
        ...
        // ... and Bucket.
        bytes memory numFieldsPerBucket = _numFieldsPerBucket(
            coordinates.bucket.storageId
        );
     ...
     */
    function _numFieldsPerBucket(
        uint256 storageId
    ) private pure returns (bytes memory) {
        if (storageId == 0) {
            return
                hex"14020202020101010101010202020202020202020101010101010102010101";
        }

        if (storageId == 1) {
            return
                hex"02010202020202010201020202020202020202020202020202020202020202020202020707";
        }

        if (storageId == 2) {
            return
                hex"0606060606080a050503010103030304030402020404010302040301";
        }
